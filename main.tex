...
\begin{abstract}
  ...
  (LSE-40 and SMTN-002) with \code{Minion\_1016}.

  % We show that these values have been revised very
  % significantly over the last few months, and we show how the depth of
  % the average visit vary with the instrument model and observing
  % conditions.

  % In order to evaluate the cost, in observing time, of the LSST SN
  % survey, it is essential to know whether the instrument model
  % currently implemented in OpSim is a conservative assessement of
  % what the survey will deliver.
