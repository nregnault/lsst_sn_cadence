...
\begin{abstract}
  ...
  efficiently years of survey operations. It is simple enough to be
  ...
  exsept in $z$.  We are currently evaluating the rolling cadence that
...
($X_1=-2, C=0.2$) our standard bright SN ($X_1=2, C=-0.2$) and our average SN ($X_1=0, C=0$). These are the three fiducial events
...
as we know) is presented in \cite[][hereafter SMTN-002)]{SMTN-002}. It
is the current baseline throughput for LSST. Telescope components are
available in github repositories
https://github.com/lsst/throughput/baseline or https://github.com/lsst-pst/sysgen\_throughputs).
...
In the cadence file \code{Minion\_1016} produced with \code{OpSim
  v3.3.5}, the five sigma depth $m_5$ is estimated from an parametric equation (see chapter 8 in \cite{SMTN-002}) taking observing conditions
(skybrightness, airmass, seeing, exposure time) into
account. Parameters related to filters are adjusted so as to get $m_5$
estimated values equal to the ones computed from \code{SMTN-002} throughput. The sky brightness is
estimated from the model described in \cite{1991PASP..103.1033K}.
The median sky obtained from the \code{Minion\_1016} logs is (expectedly) brigher
...
\begin{equation}
  ...
  \label{eqn:snr}
\end{equation}
This metric is easy to compute, all it takes is a tabulated model of
...
As stated above, 5-$\sigma$ depth values in \code{OpSim} are estimated
using a parametric equation (see chapter 8 of \cite{SMTN-002}) and
should tend to $m_5$ values computed from throughput (see eq. 41 in
\cite{LSE-40}). We have checked the precision of this equation by
estimating the difference $\Delta m_{5} = m_{5}^{throughput}-m_{5}^{OpSim}$
for each band using a wide field (id: 1981). The result
(fig. \ref{fig:m5_diff}) tend to show a compatibility better than 0.1
mag for g,r,i,z, and y band. One may also observe that $m_{5}^{OpSim}$
is systematically lower than $m_{5}^{throughput}$

\begin{figure*}[t]
\begin{center}
\includegraphics[width=0.8\linewidth]{m5_diff.png}
\caption{$\Delta m_5 = m_{5}^{throughput}-m_{5}^{OpSim}$ as a function of
  the band. $m_{5}^{throughput}$ is estimated using \code{SMTN-002}
  throughput (eq. 41 in \cite{LSE-40}), whereas $m_{5}^{OpSim}$ is
  computed using the parametric equation of chapter 8 in \cite{SMTN-002}}
\label{fig:m5_diff}
\end{center}
\end{figure*}

There are two ways to estimate a signal-to-noise ratio for a known
magnitude signal. One is to use the throughput of the instrument (see
eq. 41 of \cite{LSE-40}) and the other is to use $m_5$ with equation 5
of \cite{2008arXiv0805.2366I}. We have also checked
(fig. \ref{fig:ratio_snr}) that the resulting signal-to-noise
ratios were compatible at better than 0.15\%.

\begin{figure*}[t]
\begin{center}
\includegraphics[width=0.8\linewidth]{ratio_snr.png}
\caption{Signal-to-noise ratio (SNR$_{throughput}$/SNR$_{m_5}$) as a
  function of the magnitude difference $\Delta m = m-m_5$ for
  u,g,r,i,z, and y bands. SNR$_{throughput}$ is estimated using
  \code{STMN-002} throughput and eq. 41 of \cite{LSE-40}. SNR$_{m_5}$
  is estimated from eq. 5 of \cite{2008arXiv0805.2366I}}
\label{fig:ratio_snr}
\end{center}
\end{figure*}
